...
class ToggleTrapServos(commands2.CommandBase):  # change the name for your command

    def __init__(self, container, climber, force=None) -> None:
        super().__init__()
        self.setName('Toggle trap servos')  # change this to something appropriate for this command
    ...
    def initialize(self) -> None:
        ...
        if self.force=='on':
            ...
            self.climber.toggle_trap_servo()
            # print(f"TOGGLING SERVOS FROM {self.getName()} ELIF NONE")
        else:
            self.climber.toggle_trap_servo()
            # print(f"TOGGLING SERVOS FROM {self.getName()} ELSE")
