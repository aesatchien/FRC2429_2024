...
class AutoShootCycle(commands2.SequentialCommandGroup):
    def __init__(self, container, go_to_intake=False) -> None:
        super().__init__()
        ...
        if go_to_intake:  # This only works in teleop.  3/13/14 lhack we don't use this anymore since only our shooter position (and not intake position) lets us go under the stage
