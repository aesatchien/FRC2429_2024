...
class ShooterArm(Subsystem):
    # ShooterArm should probably have three positions that we need to map out
    positions = {'rest': 30, 'amp': 90,'trap': 180}  # todo: might need to set "intake" position
    ...
    def __init__(self):
        ...
        # initialize motors
        self.shooter_arm_controller = rev.CANSparkMax(constants.k_crank_motor_port, rev.CANSparkMax.MotorType.kBrushless)

        # self.wrist_controller.restoreFactoryDefaults()
        self.shooter_arm_controller.setIdleMode(mode=rev.CANSparkMax.IdleMode.kBrake)
        self.shooter_arm_controller.setInverted(True)  # verified that this is true for the directions we want on crank arm
        self.sparkmax_encoder = self.shooter_arm_controller.getEncoder()
        ...
        self.pid_controller = self.shooter_arm_controller.getPIDController()

        # set soft limits - do not let spark max put out power above/below a certain value
        self.shooter_arm_controller.enableSoftLimit(rev.CANSparkMax.SoftLimitDirection.kForward, constants.k_enable_soft_limits)
        self.shooter_arm_controller.enableSoftLimit(rev.CANSparkMax.SoftLimitDirection.kReverse, constants.k_enable_soft_limits)
        self.shooter_arm_controller.setSoftLimit(rev.CANSparkMax.SoftLimitDirection.kForward, self.max_angle)
        self.shooter_arm_controller.setSoftLimit(rev.CANSparkMax.SoftLimitDirection.kReverse, self.min_angle)
        self.shooter_arm_controller.setSmartCurrentLimit(20)
        self.pid_controller.setSmartMotionAllowedClosedLoopError(1)

        configure_sparkmax(sparkmax=self.shooter_arm_controller, pid_controller=self.pid_controller, slot=0, can_id= constants.k_crank_motor_port,
                           pid_dict=constants.k_PID_dict_vel_crank_arm, pid_only=True, burn_flash=constants.k_burn_flash)

        self.abs_encoder = self.shooter_arm_controller.getAbsoluteEncoder(encoderType=rev.SparkMaxAbsoluteEncoder.Type.kDutyCycle)
        ...
        SmartDashboard.putNumber('shooter_arm_setpoint', self.setpoint)
    ...
    def set_crank_arm_angle(self, angle, mode='smartmotion'):
        ...
        self.setpoint = angle
        SmartDashboard.putNumber('shooter_arm_setpoint', angle)
    ...
    def set_driver_flag(self, state=False):
        ...
        SmartDashboard.putBoolean('shooter_arm_in_use', state)
    ...
    def periodic(self) -> None:
        ...
        if self.counter % 25 == 0:
            ...
            SmartDashboard.putNumber('shooter_arm_angle', self.angle)
            SmartDashboard.putNumber('shooter_arm_abs_encoder', self.abs_encoder.getPosition())
