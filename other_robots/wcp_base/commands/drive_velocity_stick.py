...
class DriveByJoystickVelocity(commands2.CommandBase):
        ...
        if self.control_type == 'velocity':
            ...
            if (thrust**2 + twist**2)**0.5 > 0.05:
                for controller, velocity, multiplier in zip(self.drive.pid_controllers, velocities, self.multipliers):
                    controller.setReference(velocity * multiplier * slowmode_multiplier, rev.CANSparkMax.ControlType.kSmartVelocity, 1)
            else:
                for controller in self.drive.pid_controllers:
                    controller.setReference(0, rev.CANSparkMax.ControlType.kSmartVelocity, 1)
        else:  # arcade drive
    ...
    def end(self, interrupted: bool) -> None:
        ...
        for controller in self.drive.pid_controllers:
            controller.setReference(0, rev.CANSparkMax.ControlType.kSmartVelocity, 0)
