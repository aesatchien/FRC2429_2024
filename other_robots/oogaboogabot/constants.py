...
k_top_crank_abs_encoder_position_conversion_factor = 117 * math.pi  # shooter crank is 1:1 with thru-bore encoder
...
k_shooter_arm_dict = {
    'name': 'upper_arm',
    'max_angle': 109, 'min_angle': 0,  # was -79 before Q7
    'motor_can_id': 2,
    'encoder_position_conversion_factor': 1/234 * math.tau,  # shooter crank is 1:1 with thru-bore encoder,
     # 'k_kArmOffsetRads': -1.5,  # # The offset of the arm from the horizontal in its neutral position, measured from the horizontal.  Not used 4/13/24
    'k_MaxVelocityRadPerSecond': 5,  # was 3 starting Tempe (but we added a spring)
    'k_MaxAccelerationRadPerSecSquared': 5,  # was 3 starting Tempe
    'k_kSVolts': 1.0,  # was 0.3 before chain # not estimated by recalc, so we have to make something up
    'k_kGVolts': 0.53, # / 2,  # cuts in half with two motors, goes up with mass and distance, down with efficiency
    'k_kVVoltSecondPerRad': 2.29,  # stays the same with one or two motors, based on the NEO itself and gear ratio
    'k_kAVoltSecondSquaredPerRad': 0.03 , # / 2, # cuts in half with two motors
