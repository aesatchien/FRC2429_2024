...
class UpperCrankArmTrapezoidal(commands2.TrapezoidProfileSubsystem):
    ...
    def __init__(self) -> None:
        ...
        self.encoder.setInverted(True)  # needs to be inverted - clockwise on the right arm cranks the shooter down
        # crank abs encoder is 1:1 to the arm
        pos_factor = self.config['encoder_position_conversion_factor']  # 2pi
        self.encoder.setPositionConversionFactor(pos_factor)  # radians,
        self.encoder.setVelocityConversionFactor(pos_factor / 60)  # radians per second
        self.encoder.setPosition(0)
        initial_position = sum([self.encoder.getPosition() for i in range(5)]) / 5
        boot_message = f'{self.getName()} absolute encoder position at boot: {initial_position}'
        boot_message += f'set to {self.encoder.getPosition() * 180 / math.pi:.1f} degrees'
        print(boot_message)
    ...
    def periodic(self) -> None:
        ...
        if self.counter % 10 == 0:
            ...
            self.is_moving = abs(self.encoder.getVelocity()) > 0.01  # rad per second
